...
def lstm_returns(train, test, window=10):
        
         # Compute returns
    train_r = train['Price'].pct_change().dropna()
    test_r  = test['Price'].pct_change().shift(-1)
    scaler = MinMaxScaler()
    train_scaled = scaler.fit_transform(train_r.values.reshape(-1,1))
    # Create sequences
    X = []
    y = []
...
def main():
    df = load_data(r"D:\8th Sem Project\Nifty_50_Historical_Data_ISO.csv")
